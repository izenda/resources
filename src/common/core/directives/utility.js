izendaRequire.define([
	...
	function(angular) {
		angular.module('izenda.common.core').filter('izendaReplaceString', [
			function() {
			...
			function($window, $timeout) {
				return {
					...
					link: function(scope, elem) {
						...
						var setTop = function() {
							var topHeight = 0;
							angular.element.each($parent.children(),
								function() {
						...
						$timeout(function() {
							...
							1000);
						angular.element($window).on('resize.' + Math.random(),
							function() {
								setTop();
							});
					}
		...
		angular.module('izenda.common.core').directive('izendaIntegerInput', [
			function() {
				return {
					restrict: 'A',
					link: function($scope, $element) {
						$element.on('keydown', function(e) {
							if (([46, 8, 9, 27, 13].indexOf(e.keyCode) >= 0) // Allow: backspace, delete, tab, escape, enter
								||
