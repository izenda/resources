izendaRequire.define([
	...
	function izendaSelectReportNameComponentCtrl($q, $izendaLocale, $izendaUrl, $izendaSettings, $izendaCommonQuery,
			reportNameInputPlaceholderText, reportNameEmptyError, reportNameInvalidError) {
		var $ctrl = this;
		$ctrl.$izendaUrl = $izendaUrl;

		$ctrl.isCategoryAllowed = $izendaSettings.getCommonSettings().showCategoryTextboxInSaveDialog;
		$ctrl.reportNameInputPlaceholderText = $izendaLocale.localeText(reportNameInputPlaceholderText[0], reportNameInputPlaceholderText[1]);
			...
			$izendaCommonQuery.getReportSetCategory($ctrl.UNCATEGORIZED_TEXT).then(function (data) {
				...
				if (angular.isArray($ctrl.reportSets)) {
					$ctrl.reportSets.forEach(function (report) {
						...
						currentCategoryName = !report.Subcategory ? currentCategoryName : currentCategoryName + $izendaSettings.getCategoryCharacter() + report.Subcategory;
						if (angular.element.grep($ctrl.categories, function (a) { return a.name === currentCategoryName; }).length === 0) {
		...
		$ctrl.validateForm = function () {
			return $q(function (resolve, reject) {
				...
				if (reportInfo.name === $ctrl.reportName && reportInfo.category === selectedCategoryName) {
					$ctrl.errorMessages.push($ctrl.ERROR_REPORT_EXIST(selectedCategoryName + $izendaSettings.getCategoryCharacter() + $ctrl.reportName));
					reject();
					return false;
				}
		...
		function getCategoryByName(name) {
			var category = null;
			angular.element.each($ctrl.categories, function () {
				if (this.name === name)
					category = this;
			});
			return category;
