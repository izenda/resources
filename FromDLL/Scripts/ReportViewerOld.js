...
function ChangeTopRecords(recsNum) {
    ...
    if (recsNum == 1) {
        ...
        resNumImg.src = '###RS###image=ModernImages.' + 'row-1.png';
    }
    if (recsNum == 10) {
        uvcVal = '10';
        $('#resNumLi1').addClass('selected');
        resNumImg.src = '###RS###image=ModernImages.' + 'rows-10.png';
    }
    if (recsNum == 100) {
        uvcVal = '100';
        $('#resNumLi2').addClass('selected');
        resNumImg.src = '###RS###image=ModernImages.' + 'rows-100.png';
    }
    if (recsNum == 1000) {
        uvcVal = '1000';
        $('#resNumLi3').addClass('selected');
        resNumImg.src = '###RS###image=ModernImages.' + 'rows-1000.png';
    }
    if (recsNum == 10000) {
        uvcVal = '10000';
        $('#resNumLi5').addClass('selected');
        resNumImg.src = '###RS###image=ModernImages.' + 'rows-10000.png';
    }    
    if (recsNum == -1) {
        uvcVal = '-1';
        $('#resNumLi4').addClass('selected');
        resNumImg.src = '###RS###image=ModernImages.' + 'rows-all.png';
    }
    var fieldWithPCId = document.getElementById('previewControlIdFor2ver');
    var contId = fieldWithPCId.value;
    filtersTrickBeforeUpdate();
