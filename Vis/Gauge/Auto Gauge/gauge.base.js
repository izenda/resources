...
(function (p) {
	...
	p.formatNumber = function (value ,format) {
		...
		return p.accounting.formatNumber(value, format.precision);
	};

	p.formatCurrency = function (value, format) {
		format = jq$.extend(p.defaultFormat(), format);
		return p.accounting.formatMoney(value, format.currency, format.precision);
	};
